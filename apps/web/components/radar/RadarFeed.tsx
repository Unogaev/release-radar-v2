    ...
    <div className="min-h-screen max-w-full overflow-x-hidden bg-[#0b0b0c] font-rr-sans text-[#f3efe8]">
      ...
      <main className={`mx-auto max-w-[1500px] px-4 pb-20 pt-6 transition-opacity sm:px-8 sm:pt-8 lg:px-12 ${isPending ? "opacity-50" : ""}`}>
        ...
        {error && !payload ? <FeedError reason={error} onRetry={onRetry} /> : <>
          ...
          <section className="mt-1 sm:mt-5">{visible.length ? <div><SignalHero signal={hero} now={now} onAction={(id, action) => onAction?.(id, action)} />{rest.length > 0 && <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{rest.map((s) => <SignalCard key={s.id} signal={s} now={now} onAction={(id, action) => onAction?.(id, action)} />)}</div>}</div> : <div className="grid overflow-hidden rounded-[20px] border border-white/10 bg-[#121212] lg:grid-cols-[1.25fr_.75fr]"><div className="relative p-5 sm:p-9"><div className="absolute right-0 top-0 h-64 w-64 rounded-full bg-[#c6a66b]/[.06] blur-3xl" /><div className="relative max-w-[650px]"><div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-[#c6a66b]/10 text-[#c6a66b] sm:mb-5 sm:h-11 sm:w-11 sm:rounded-2xl"><ShieldCheck size={20} /></div><h2 className="font-rr-editorial text-[28px] font-semibold leading-[1.05] tracking-[-0.025em] sm:text-4xl">{c.noAction}</h2><p className="mt-3 text-[13px] leading-5 text-white/48 sm:text-sm sm:leading-6">{c.noActionText}</p><button onClick={() => setFilter("soon")} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#ece5d9] px-5 py-3 text-xs font-bold text-[#171513] sm:mt-6 sm:w-auto">{c.noActionCta}<ArrowUpRight size={14} /></button></div></div><div className="hidden border-t border-white/10 bg-black/20 p-7 sm:block lg:border-l lg:border-t-0"><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-white/35">{lang === "ru" ? "Жёсткие условия сделки" : "Hard deal gates"}</div><div className="mt-5 space-y-4 text-sm">{(lang === "ru" ? ["Продавец и checkout подтверждены", "Закупка с налогом рассчитана", "Completed sales отделены от asks", "Чистая маржа проходит порог"] : ["Seller and checkout verified", "Taxed acquisition cost calculated", "Completed sales separated from asks", "Net margin clears the threshold"]).map((gate) => <div key={gate} className="flex items-start gap-3 text-white/62"><span className="mt-1 h-2 w-2 shrink-0 rounded-full border border-[#c6a66b]/60" />{gate}</div>)}</div></div></div>}</section>
          ...
          <section className="mt-12 flex flex-col justify-between gap-4 rounded-[20px] border border-white/10 bg-white/[.025] p-5 sm:flex-row sm:items-center"><div><div className="text-sm font-semibold">{c.sourceHealth}</div><div className="mt-1 text-xs text-white/40"><span className="text-[#8fe0aa]">{healthySources} {c.sourcesOk}</span><span> · {manualSources} {lang === "ru" ? "ручных официальных проверок" : "manual official checks"}</span><span> · {payload?.sources.length ?? 0} {lang === "ru" ? "всего" : "total"}</span>{troubledSources > 0 && <span> · {troubledSources} {c.sourcesIssue}</span>}</div></div><Link href="/sources" className="inline-flex items-center gap-2 text-xs font-semibold text-white/60">{c.source}<ArrowUpRight size={13} /></Link></section>
        </>}
      </main>
    </div>
  );
