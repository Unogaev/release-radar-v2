...
export default async function PurchasesPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="font-display text-2xl text-rr-text">Мои покупки</h1>
      ...
      {purchases.length === 0 ? (
        ...
        <table className="w-full text-sm">
          <thead className="text-left text-rr-text-dim border-b border-rr-hair">
            <tr>
          ...
          <tbody>
            {purchases.map((p) => (
              <tr key={p.id} className="border-b border-rr-hair">
                <td className="py-3">
                ...
                <td className="text-rr-text-dim">{p.status}</td>
                <td>
